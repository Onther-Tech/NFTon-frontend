...
var CrosshairPlugin = {
  ...
  drawTraceLine: function (chart) {
    ...
    for (var chartIndex = 0; chartIndex < chart.data.datasets.length; chartIndex++) {
      ...
      chart.ctx.moveTo(lineX, yScale.getPixelForValue(dataset.interpolatedValue));
  ...
  drawTracePoints: function (chart) {
    ...
    for (var chartIndex = 0; chartIndex < chart.data.datasets.length; chartIndex++) {
      ...
      chart.ctx.shadowBlur = 2.67;
      chart.ctx.shadowOffsetY = 2.67;
      chart.ctx.shadowColor = 'rgba(50, 50, 71, 0.06)';
      chart.ctx.beginPath();
      chart.ctx.arc(chart.crosshair.x, yScale.getPixelForValue(dataset.interpolatedValue), 6, 0, 2 * Math.PI, false);
      chart.ctx.fillStyle = '#45FF79';
